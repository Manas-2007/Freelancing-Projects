...
import { useLocation, useNavigate } from 'react-router-dom'; // 1. useNavigate add kiya
...
const DashboardHeader = () => {
  ...
  const navigate = useNavigate(); // 2. Navigation trigger
    ...
    <header className="w-full h-[100px] flex-shrink-0 flex items-center justify-between px-10 bg-white/80 backdrop-blur-2xl border-b border-slate-100/80 sticky top-0 z-[100] font-sans">
      ...
      <div className="flex items-center gap-8">
        ...
        <div className="flex items-center gap-3 p-1.5 rounded-[20px] border border-slate-200/50 shadow-inner">
          ...
          <button 
            onClick={() => {
              if(window.confirm("Do you want to exit the session?")) logout();
            }} 
        ...
        {/* PROFILE SECTION - Navigation to Profile Page */}
        <div 
          onClick={() => navigate('/dashboard/profile')} 
          className="flex items-center gap-4 group cursor-pointer h-full"
        >
