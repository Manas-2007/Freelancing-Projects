...
const InfoModal = ({ isOpen, onClose, tabType }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-3 sm:p-6 animate-in fade-in duration-300">
      {/* 1. SOFT BACKDROP */}
      <div 
      ...
      {/* 2. WIDE WHITE MODAL */}
      <div className="relative w-full max-w-[700px] h-[90vh] md:h-[85vh] max-h-[650px] bg-white rounded-[28px] md:rounded-[45px] shadow-[0_40px_120px_-20px_rgba(0,0,0,0.4)] 
        ...
        <div className={`flex-shrink-0 px-5 pt-6 pb-4 md:px-12 md:pt-10 md:pb-6 border-b-4 ${content.accent} bg-white z-10`}>
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3 md:gap-5">
              <div className={`p-2.5 md:p-3.5 rounded-2xl bg-slate-50 border border-slate-100 shadow-sm transition-transform hover:rotate-6`}>
                {content.icon}
              </div>
              <h3 className="text-[18px] md:text-[26px] font-bold text-slate-900 tracking-tight leading-none">
                {content.title}
              </h3>
            </div>
            <button 
              onClick={onClose} 
              className="w-9 h-9 md:w-11 md:h-11 flex items-center justify-center hover:bg-slate-100 rounded-full text-slate-400 transition-all active:scale-90"
            >
              <X size={20} />
            </button>
          </div>
        ...
        <div className="flex-1 px-5 py-6 md:px-12 md:py-10 overflow-y-auto 
          ...
          <div className="space-y-6 md:space-y-8 animate-in fade-in duration-1000 delay-150">
            <p className="text-[15px] md:text-[18px] text-slate-800 leading-[1.6] font-semibold">
              {content.mainText}
            </p>
            
            {content.details ? (
              <div className="grid grid-cols-1 gap-3 md:gap-5 mt-4 md:mt-6">
                {content.details.map((item, i) => (
                  <div key={i} className="flex gap-3 md:gap-4 p-3 md:p-5 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-blue-200 transition-all">
                    <div className="mt-1 p-2 bg-white rounded-lg shadow-sm text-blue-600 flex-shrink-0">
                        {item.icon}
                    </div>
                    <div>
                        <span className="text-slate-900 font-bold text-[13px] md:text-[15px] block mb-1">{item.label}</span>
                        <span className="text-slate-600 text-[12px] md:text-[13.5px] font-medium leading-relaxed block">{item.text}</span>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="relative pl-5 md:pl-8 py-2">
                <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${content.accent.replace('border','bg')} rounded-full opacity-60`}></div>
                <p className="text-[14px] md:text-[16px] text-slate-600 leading-relaxed font-medium italic">
                    {content.subText}
                </p>
              </div>
            )}
        ...
        <div className="flex-shrink-0 px-5 py-4 md:px-12 md:py-8 bg-white border-t border-slate-100 z-10 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
             <div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]"></div>
             <span className="text-[10px] md:text-[12px] font-bold text-slate-400 uppercase tracking-[2px]">
               {content.footerNote}
             </span>
          </div>
          <button 
            onClick={onClose}
            className="group flex items-center gap-2 md:gap-3 bg-slate-900 text-white px-5 py-2.5 md:px-9 md:py-3.5 rounded-2xl font-bold text-[13px] md:text-[14px] hover:bg-blue-600 transition-all active:scale-95 shadow-lg shadow-slate-900/10"
          >
