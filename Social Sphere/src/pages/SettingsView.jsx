...
const SettingsView = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-[-20px] p-6 bg-slate-50/50 rounded-[35px] font-sans">
      ...
      {/* 1. APP THEME SECTION (Interactive Toggle) */}
      <SettingsCard title="App Appearance">
        <div className="space-y-5">
          <div 
            onClick={() => setDarkMode(!darkMode)}
            className={`flex items-center justify-between p-4 rounded-[22px] border-2 transition-all cursor-pointer group ${darkMode ? 'bg-slate-900 border-slate-700 shadow-xl' : 'bg-white border-blue-100 shadow-sm'}`}
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className={`font-bold text-[15px] tracking-tight ${darkMode ? 'text-white' : 'text-slate-900'}`}>Dark Mode</span>
      ...
      {/* 3. SUPPORT SECTION (Interactive Feedback) */}
      <SettingsCard title="Social Sphere Support">
        <form onSubmit={handleSubmitTicket} className="flex flex-col gap-3">
          ...
          <button 
            type="submit"
            className={`group w-full py-4 rounded-[20px] font-bold text-[15px] shadow-lg transition-all flex items-center justify-center gap-2 ${submitted ? 'bg-emerald-500 text-white shadow-emerald-200' : 'bg-blue-600 text-white shadow-blue-500/20 hover:bg-blue-700'}`}
          >
      ...
      <SettingsCard title="Alerts & Notifications">
        <div className="space-y-3">
           <div 
            ...
            className={`flex items-center justify-between p-3.5 rounded-xl border cursor-pointer transition-all ${pushNotifs ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-100'}`}
           >
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-lg shadow-sm transition-colors ${pushNotifs ? 'bg-blue-600 text-white' : 'bg-white text-slate-400'}`}>
                  <BellRing size={18}/>
                </div>
                <span className={`font-bold text-[14px] transition-colors ${pushNotifs ? 'text-blue-700' : 'text-slate-500'}`}>Push Notifications</span>
              </div>
      ...
      {/* AGGRESSIVE LOGOUT BUTTON (Functional) */}
      <div className="flex justify-center lg:col-span-2 pt-4">
        <button 
          onClick={handleLogout}
          className="group relative flex items-center gap-4 bg-[#0f172a] px-12 py-4 rounded-[25px] shadow-xl border border-white/5 hover:bg-red-600 transition-all duration-500 active:scale-95"
        >
